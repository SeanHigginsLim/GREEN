 ...
 * 
 * NOTE: This is only used for TESTING purposes. Production code uses
 * RAGEngine.getKnowledgeGraph() which fetches pre-computed edges and weights
 * from Firebase, not calculated distances.
 */
class GraphBuilder {
    companion object {
                ...
                if (i != j) {
                    ...
                    if (distance <= MAX_WALKING_DISTANCE_KM) {
                        ...
                        adjacencyList[poi1.poiId]?.add(
                            Edge("edge_${poi1.poiId}_${poi2.poiId}", poi2.poiId, weight)
                        )
                    }
                }
            }
